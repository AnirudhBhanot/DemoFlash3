...
# Import the intelligent framework API
from api_framework_intelligent import include_intelligent_routes
from api_michelin_resilient import router as michelin_resilient_router
from api_michelin_enhanced import enhanced_router, shutdown_enhanced_engine
...
app.include_router(michelin_resilient_router)
...
# Include Framework Implementation endpoints (Michelin format)
try:
    from api_framework_implementation import implementation_router
    app.include_router(implementation_router)
    logger.info("Framework Implementation endpoints enabled (Michelin format)")
except ImportError as e:
    logger.warning(f"Could not import Framework Implementation endpoints: {e}")

# Include Framework Adapter endpoints for startup data
try:
    from api_framework_adapter import adapter_router
    app.include_router(adapter_router)
    logger.info("Framework Adapter endpoints enabled for startup data conversion")
except ImportError as e:
    logger.warning(f"Could not import Framework Adapter endpoints: {e}")

# Include Framework Analysis endpoints (actual framework application)
try:
    from api_framework_analysis import analysis_router
    app.include_router(analysis_router)
    logger.info("Framework Analysis endpoints enabled - applies frameworks to data")
except ImportError as e:
    logger.warning(f"Could not import Framework Analysis endpoints: {e}")

# Include Enhanced Framework Analysis endpoints (intelligent LLM-based selection)
try:
    from api_framework_analysis_endpoints import framework_analysis_router
    app.include_router(framework_analysis_router)
    logger.info("Enhanced Framework Analysis endpoints enabled - intelligent LLM-based framework selection")
except ImportError as e:
    logger.warning(f"Could not import Enhanced Framework Analysis endpoints: {e}")

# Include Deep Framework Analysis endpoints (McKinsey-quality analysis)
try:
    from api_framework_deep_analysis import deep_analysis_router
    app.include_router(deep_analysis_router)
    logger.info("Deep Framework Analysis endpoints enabled - McKinsey-quality strategic analysis")
except ImportError as e:
    logger.warning(f"Could not import Deep Framework Analysis endpoints: {e}")

# Include Hybrid Michelin endpoints (DeepSeek with automatic fallback)
# DISABLED: Conflicts with LLM Analysis router (same prefix)
# try:
#     from api_michelin_hybrid import hybrid_router
#     app.include_router(hybrid_router)
#     logger.info("Michelin Hybrid endpoints enabled - DeepSeek with automatic fallback")
# except ImportError as e:
#     logger.warning(f"Could not import Michelin Hybrid endpoints: {e}")

# Include Michelin Strategic Analysis endpoints
# DISABLED: Conflicts with LLM Analysis router (same prefix)
# try:
#     from api_michelin_strategic_analysis import michelin_router
#     app.include_router(michelin_router)
#     logger.info("Michelin Strategic Analysis endpoints enabled - 3-phase strategic framework")
# except ImportError as e:
#     logger.warning(f"Could not import Michelin Strategic Analysis endpoints: {e}")

# Include Fixed Michelin Analysis endpoints
try:
    from api_michelin_fixed import fixed_michelin_router
    app.include_router(fixed_michelin_router)
    logger.info("Fixed Michelin Analysis endpoints enabled - handles DeepSeek JSON properly")
except ImportError as e:
    logger.warning(f"Could not import Fixed Michelin Analysis endpoints: {e}")

# Include Working Michelin Analysis endpoints
try:
    from api_michelin_working import working_michelin_router
    app.include_router(working_michelin_router)
    logger.info("Working Michelin Analysis endpoints enabled - immediate responses without DeepSeek")
except ImportError as e:
    logger.warning(f"Could not import Working Michelin Analysis endpoints: {e}")

# Include Michelin LLM Analysis endpoints (for completeness)
try:
    from api_michelin_llm_analysis import michelin_router as llm_michelin_router
    app.include_router(llm_michelin_router)
    logger.info("Michelin LLM Analysis endpoints enabled - DeepSeek with improved JSON parsing")
except ImportError as e:
    logger.warning(f"Could not import Michelin LLM Analysis endpoints: {e}")

# Include Decomposed Michelin Analysis endpoints
try:
    from api_michelin_decomposed import decomposed_router
    app.include_router(decomposed_router)
    logger.info("Decomposed Michelin Analysis endpoints enabled - Reliable multi-step approach")
except ImportError as e:
    logger.warning(f"Could not import Decomposed Michelin Analysis endpoints: {e}")

# Include Strategic Michelin Analysis endpoints
try:
    from api_michelin_strategic import strategic_router
    app.include_router(strategic_router)
    logger.info("Strategic Michelin Analysis endpoints enabled - Redesigned with proper structure and phase interconnection")
except ImportError as e:
    logger.warning(f"Could not import Strategic Michelin Analysis endpoints: {e}")

# Include Deep Framework Analysis endpoints
try:
    from api_framework_deep_analysis import deep_analysis_router
    app.include_router(deep_analysis_router)
    logger.info("Deep Framework Analysis endpoints enabled - detailed strategic insights with DeepSeek")
except ImportError as e:
    logger.warning(f"Could not import Deep Framework Analysis endpoints: {e}")

# Enhanced Michelin Analysis endpoints
try:
    app.include_router(enhanced_router)
    logger.info("Enhanced Michelin Analysis endpoints enabled - McKinsey-grade strategic analysis")
except Exception as e:
...
try:
    ...
    logger.info("Intelligent Framework endpoints enabled - ML-powered framework selection with real-time intelligence")
except Exception as e:
...
async def predict(
    ...
    # current_user: CurrentUser = Depends(get_current_user_flexible)  # TEMPORARILY DISABLED FOR DEVELOPMENT
):
...
if __name__ == "__main__":
    ...
    async def shutdown():
        ...
        # Shutdown deep analysis engine if available
        try:
            from api_framework_deep_analysis import shutdown_deep_analysis_engine
            logger.info("Shutting down deep analysis engine...")
            await shutdown_deep_analysis_engine()
        except ImportError:
            pass
        
        # Shutdown Michelin engine if available  
        try:
            from api_michelin_llm_analysis import shutdown_michelin_engine
            logger.info("Shutting down Michelin engine...")
            await shutdown_michelin_engine()
        except ImportError:
            pass
            
        # Shutdown Enhanced Michelin engine if available
        try:
            logger.info("Shutting down Enhanced Michelin engine...")
            await shutdown_enhanced_engine()
        except ImportError:
            pass
        
        # Shutdown Decomposed Michelin engine if available
        try:
            from api_michelin_decomposed import shutdown_decomposed_engine
            logger.info("Shutting down Decomposed Michelin engine...")
            await shutdown_decomposed_engine()
        except ImportError:
            pass
        
        # Shutdown Strategic Michelin engine if available
        try:
            from api_michelin_strategic import shutdown_strategic_engine
            logger.info("Shutting down Strategic Michelin engine...")
            await shutdown_strategic_engine()
        except ImportError:
            pass
